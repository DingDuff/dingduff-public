...
- Aim for **5–15 cases per phrase**, weighted to most-cited and most-recent. Triage from snippets; bulk-retrieve via `fetch_opinion_file`; save to `saved cases/`; read.
...
- `opinion_search` / `fetch_opinion_file` / `opinion_view` / `show_citing_opinions` — step 5 interpreting cases (see `case-law.md`).
