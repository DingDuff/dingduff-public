...
| **Validity Check** — [`download`](dist/dingduff_validity-check_v3.6.skill) | v3.6 | Before an argument rests on a case, confirms it is still good law **for the proposition you're citing it for**: reads the **docket** for federal cases to see whether the decision itself was appealed, reversed, vacated, or amended; sweeps forward for overruling and erosion; checks the authorities the case leaned on; and looks for a rule that moved without anyone citing it. Returns a verdict with its confidence and its coverage limits. Legal Research calls it automatically on load‑bearing cases; you can also point it at one case yourself. |
